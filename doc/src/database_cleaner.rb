  ...
  class << self
    ...
    private
      ...
      def symlink_database
        %x(mv #{current_location}/db/database.sqlite 
          #{shared_location}/db/database.sqlite)
        
        %x(ln -s #{shared_location}/db/database.sqlite 
            #{current_location}/db/database.sqlite)
      end
